...
export const SizeForm: React.FC<SizeFormProps> = ({ initalData }) => {
	...
	const title = initalData ? 'Modifier le modèle' : 'Créer un modèle';
	const description = initalData
		? 'Editer une description'
		: 'Ajouter un description';
	const toastMessage = initalData ? 'Modèle modifié.' : 'Modèle créé.';
	...
	const onDelete = async () => {
		try {
			...
			toast.error(
				"Assurez-vous d'abord d'avoir supprimé tous les produits utilisant ce modèle"
			);
		} finally {
			setLoading(false);
			setOpen(false);
